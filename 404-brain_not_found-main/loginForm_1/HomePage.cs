...
namespace loginForm_1
{
    public partial class HomePage : Form
    {
        ...
        private string activeUsername = "Guest";
        ...
        public void setCurrentUserloggedin(string username)
        {
            ...
            {
                lblWelcomeUser.Text = $"Live: {activeUsername} 🚀";
            }

            // Reload user-specific playlists upon successful login
            userPlaylists.Clear();
            InitializeDefaultPlaylists();
            LoadSavedPlaylists();
            DisplayFavoritePlaylists();
        ...
        {
            string cleanUsername = string.Concat(activeUsername.Split(Path.GetInvalidFileNameChars()));
            return $"saved_playlists_{cleanUsername}.json";
        }
        ...
        public void SaveCustomPlaylists()
        {
            try
            {
                ...
                string json = JsonSerializer.Serialize(customPlaylists, options);
                File.WriteAllText(GetUserSaveFilePath(), json);
            }
            catch (Exception ex)
            {
        ...
        private void LoadSavedPlaylists()
        {
            try
            {
                string filePath = GetUserSaveFilePath();
                if (File.Exists(filePath))
                {
                    ...
                    if (customPlaylists != null)
                    {
                        foreach (var playlist in customPlaylists)
                        {
                            ...
                            {
                                playlist.CoverImage = LoadImageSafely(playlist.CoverImagePath);
                            }
        ...
        private Image LoadImageSafely(string fileName)
        {
            try
            {
                string targetPath = File.Exists(fileName) ? fileName : Path.Combine(AppDomain.CurrentDomain.BaseDirectory, fileName);
        ...
        private void AddCreatePlaylistCard()
        {
            ...
            {
                Size = new Size(134, 109),
                Location = new Point(53, 3),
                SizeMode = PictureBoxSizeMode.Zoom,
                BackColor = Color.FromArgb(50, 35, 75),
                Image = LoadImageSafely("Beaverswood Warehouse Floor Markers _ Facilities Management _ BiGDUG.jpg")
            };
            ...
            Button btnCreate = new Button
            {
                ...
                Location = new Point(53, 117),
                FlatStyle = FlatStyle.Flat,
                ForeColor = Color.White,
                BackColor = Color.BlueViolet
            };

            btnCreate.Click += (s, ev) => OpenForm2();
        ...
        {
            Form2 createPlaylistForm = new Form2(this);
        ...
        public void UpdateStatisticalInsights()
        {
            ...
            int totalTracks = userPlaylists.Where(p => p.Songs != null).Sum(p => p.Songs.Count);
